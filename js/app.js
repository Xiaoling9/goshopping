  ...
  $(document).ready(function(){
  	...
  	//userinfo page username show
  	var username_userinfo="";
	username_userinfo  += "<p class=\"nametext\" style=\"margin-left:20px;\">" + Bmob.User.current().getUsername() +" <\/p>";
	$("#username_content").append(username_userinfo);
   ...
   function postClick(){
	var myproductname = $("#productnameLable").val();
	var uploadimg = $("#uploadImg")[0];
	if(uploadimg.files.length > 0){
		var mypostpic = uploadimg.files[0];
	...
	mypost.set("productName",myproductname);
	mypost.set("star",myproductgrade);
	mypost.set("postContent",mypostcontent);
	mypost.set("username",currentUser);
	mypost.set("postPic",file);
	// alert(mypostpic);
	mypost.save(null, {
	...
	// else{
	// 	alert("请登录！");
	// 	top.window.location.href="../html/login.html";
	// }
